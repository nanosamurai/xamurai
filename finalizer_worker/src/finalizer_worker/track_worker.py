...
def consume(consumer, handler, *, stopped=lambda: False):
    ...
    consumer.subscribe([os.getenv("KAFKA_TOPIC_RECORDING_FINISHED", "recordings.finished")],
...
def main():
    ...
    track_id = require_name(os.getenv("FINAL_TRACK_ID", "whisperx"))
    profile_id = require_name(os.getenv("FINAL_PROFILE_ID", "whisperx-medium-final-r1"))
    if profile_id == "whisperx-medium-final-r1":
        from finalizer_worker.whisperx_track import WhisperXTrack
        provider = WhisperXTrack()
    elif profile_id == "test-final-r1" and os.getenv("FINAL_TRACK_TEST_PROFILE_ENABLED") == "true":
    ...
    consumer = Consumer(dict(config, **{
        "group.id": f"final-track.{track_id}.{profile_id}",
