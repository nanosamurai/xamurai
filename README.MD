...
- Its optional [Parakeet final track](docs/parakeet-finalizer.md) reuses that loop
  with Parakeet TDT 0.6B v3 and embedded Sortformer, including word timestamps
  and up to four anonymous speakers. Build the `parakeet-finalizer` target in
  `nemotron_rtservice/Dockerfile`; no additional inference service is required.
- `nemo_speech_native` provides the shared NeMo-Speech.cpp bindings, runtime
  checks and verified model downloads used by Nemotron and Parakeet.
...
| `finalizer_worker.parakeet` | Kafka completed-recording pipeline | Parakeet TDT 0.6B v3 Q8 through NeMo-Speech.cpp; native word timing | Embedded Sortformer v2 Q8 | Independent final track with up to four anonymous speakers |
...
- `NEMOTRON_ENDPOINTING_SILENCE_MS` (default `2000`; integer `1`–`30000`):
  decoder-silence timeout, read at startup; invalid values fail startup.
...
Nemotron uses NeMo-Speech.cpp's native token-silence endpointing (2000 ms by
default) so each utterance is committed independently. Set
`NEMOTRON_ENDPOINTING_SILENCE_MS=800` to restore the previous timeout, or
...
`WHISPERX_MODEL` supplies the existing `refinement_model` field.
